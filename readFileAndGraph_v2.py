...
def create_and_save_graph(data_for_single_client, graph_title_main_part, year, output_html_path, is_archive_file=False):
    """
    Crea un grafico a barre per un singolo client e lo salva come file HTML interattivo.
    data_for_single_client: Dizionario con dati per un solo client, es. {client_id: {"days": [...], "values": [...]}}
    graph_title_main_part: Stringa come "Ottobre 2023 (Client: NomeClient)"
    is_archive_file: Boolean, True se è un grafico di un mese archiviato.
    """
    if not data_for_single_client or not list(data_for_single_client.values())[0].get("days"):
        logger.warning(f"Nessun dato o nessun giorno con dati fornito per generare il grafico: {graph_title_main_part} in {output_html_path}.")
        html_content = f"<html><head><title>Grafico Consumi {graph_title_main_part}</title></head>"
        html_content += f"<body><h1>Grafico Consumi Acqua - {graph_title_main_part}</h1>"
        html_content += f"<p>Nessun dato disponibile per {graph_title_main_part}.</p>"
    else:
        client_id = list(data_for_single_client.keys())[0]
        client_specific_data = data_for_single_client[client_id]

        fig, ax = plt.subplots(figsize=(20, 10))
        
        all_days_sorted = sorted(list(set(client_specific_data["days"])))
        if not all_days_sorted: # Controllo aggiuntivo se "days" fosse vuoto ma non "data_for_single_client"
            logger.warning(f"Nessun giorno con dati trovato per {graph_title_main_part} (Client: {client_id}).")
            html_content = f"<html><head><title>Grafico Consumi {graph_title_main_part}</title></head>"
            html_content += f"<body><h1>Grafico Consumi Acqua - {graph_title_main_part}</h1>"
            html_content += f"<p>Nessun dato disponibile per {graph_title_main_part}.</p>"
        else:
            ...
            ax.set_title(f'Consumi Acqua - {graph_title_main_part}', fontsize=22, fontweight='bold', color='green')
    ...
    other_grafico_files = []
    if os.path.exists(REPO_ROOT_DIR): # Assicurati che la directory esista
        other_grafico_files = sorted(
            [f for f in os.listdir(REPO_ROOT_DIR) if f.startswith("grafico_") and f.endswith(".html") and os.path.join(REPO_ROOT_DIR, f) != output_html_path],
            reverse=True
        )
        # Aggiungi index.html se esiste e non è il file corrente
        if os.path.exists(HTML_OUTPUT_PATH) and HTML_OUTPUT_PATH != output_html_path:
            if os.path.basename(HTML_OUTPUT_PATH) not in [os.path.basename(f) for f in other_grafico_files]: # Evita duplicati se già incluso
                 other_grafico_files.append(os.path.basename(HTML_OUTPUT_PATH)) # Aggiungi solo il nome base

    if not other_grafico_files:
         html_content += "<li>Nessun altro grafico o archivio disponibile.</li>"

    for i, other_file_name in enumerate(other_grafico_files):
        if i < 24: 
             link_display_name = other_file_name.replace(".html", "").replace("grafico_", "").replace("_", " ")
             if other_file_name == "index.html":
                 link_display_name = "Pagina Principale (Nessun dato per mese corrente)"
             else:
        ...
        if match:
            ...
            if archived_month_data_all_clients:
                for client_id, client_specific_data in archived_month_data_all_clients.items():
                    ...
                    graph_main_title = f"{mese_str_archivio} {log_year} (Client: {client_id})"
...
if __name__ == "__main__":
    ...
    if not current_month_data_all_clients:
        logger.info(f"Nessun dato per il mese corrente ({current_month_name} {current_year_num}). Generazione di {HTML_OUTPUT_PATH} vuoto.")
        create_and_save_graph(
            {}, 
        ...
        for client_id, client_specific_data in current_month_data_all_clients.items():
            if not client_specific_data.get("days"): # Salta se il client non ha giorni con dati
                logger.info(f"Nessun giorno con dati per il client '{client_id}' nel mese corrente.")
                continue

            data_for_graph = {client_id: client_specific_data}
            safe_client_id = re.sub(r'[^\w\-\.]', '_', client_id)
            
            current_client_graph_filename = f"grafico_{current_year_num}-{current_month_num:02d}_{safe_client_id}.html"
            current_client_graph_filepath = os.path.join(REPO_ROOT_DIR, current_client_graph_filename)
            graph_main_title = f"{current_month_name} {current_year_num} (Client: {client_id})"
            
            create_and_save_graph(
                data_for_graph, 
                graph_main_title, 
                current_year_num, 
                current_client_graph_filepath, 
                is_archive_file=False
            )
            generated_html_files_for_git.append(current_client_graph_filepath)
